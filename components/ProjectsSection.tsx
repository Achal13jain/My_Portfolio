  ...
  const projects = [
    {
      ...
      tech: ['Python', 'FastAPI', 'PyTorch', 'NLP', 'Transformers'],
    ...
    },
    {
      ...
      description: 'Scalable microservices architecture for e-commerce platform with user management, inventory tracking, and payment processing. RESTful APIs with comprehensive testing.',
      icon: <ShoppingCart size={32} />,
      color: 'from-teal-500 to-teal-700',
      tech: ['Node.js', 'PostgreSQL', 'Redis', 'JWT', 'Stripe API'],
    ...
    },
    {
      ...
      tech: ['Python', 'Scikit-learn', 'Pandas', 'Flask', 'Bootstrap'],
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          {projects.map((project, index) => (
            ...
            >
              {/* Card Container */}
              <div className={`glass rounded-2xl p-8 h-full border ${project.border} hover:border-opacity-50 transition-all duration-500 relative overflow-hidden ${project.gradient}`}>

                {/* Background Pattern */}
                <div className="absolute inset-0 opacity-5">
                  <div className="absolute inset-0" style={{
                    backgroundImage: 'radial-gradient(circle at 25% 25%, rgba(168, 85, 247, 0.1) 0%, transparent 50%), radial-gradient(circle at 75% 75%, rgba(20, 184, 166, 0.1) 0%, transparent 50%)',
                  }}></div>
                </div>
                ...
                <div className="relative z-10">
                  ...
                  <div className="flex items-start justify-between mb-6">
                    <div className={`inline-flex items-center justify-center w-16 h-16 rounded-xl bg-gradient-to-br ${project.color} text-white group-hover:scale-110 transition-transform duration-300`}>
                      {project.icon}
                    </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {project.tech.map((tech, techIndex) => (
                      <motion.span
                        ...
                        className="px-3 py-1 text-xs font-medium bg-slate-800/80 text-slate-300 rounded-full border border-slate-600/50 group-hover:bg-slate-700/80 group-hover:text-slate-200 group-hover:border-purple-500/30 transition-all duration-300"
                      >
                        {tech}
                      </motion.span>
                    ))}
